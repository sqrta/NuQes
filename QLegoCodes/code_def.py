...
class codeTN:
    def __init__(self, stabilizers, name="", symmetry=None, isStab=False) -> None:
        if not isStab:
...
class TNNetwork:
    ...
    def trace(self, localIndex, leg1, tensor, leg2):
        ...
        if localIndex >= len(self.tensorList):
            raise ValueError(
                msg,
                f"first index {localIndex} out of tnlist length {len(self.tensorList)}",
            )
        ...
        if leg1 in local.tracted:
            ...
            raise ValueError(
                msg, f"leg {leg1} exceeds TN {localIndex}'s index (max {local.size-1})"
            )
    ...
    def selfTrace(self, index1, leg1, index2, leg2):
        ...
        if leg1 in tn1.tracted:
            ...
            raise ValueError(
                f"leg {leg1} of TN {index1} is traced. TN {index1}'s traced set {tn1.tracted}"
            )
        if leg2 in tn2.tracted:
            print("self", index1, leg1, index2, leg2)
            raise ValueError(
                f"leg {leg2} of TN {index2} is traced. TN {index2}'s traced set {tn2.tracted}"
            )
...
code604 = codeTN(
    ["ixzzxi", "iixzzx", "ixixzz", "izxixz", "xxxxxx", "zzzzzz"],
    "code604",
    symmetry=[0],
)
code603 = codeTN(
    ["iixxxx", "iizzzz", "xixxii", "ixixix", "zizizi", "iziizz"],
    "code603",
    symmetry=[0, 2],
)
code804 = codeTN(
...
code713 = codeTN(
    ["iiixxxx", "ixxiixx", "xixixix", "iiizzzz", "izziizz", "ziziziz"], "code713"
)
...
code5_1_3_m = codeTN(
    ["xiixx", "ixiix", "iixxi", "iiiii", "ziziz", "ziizz", "ziizz", "izzzz"]
)
...
code823 = codeTN(
    ["XZZXIIII", "ZXZIIIII", "IZXZIIII", "IIIIXZZX", "IIIIZXZI", "IIIIIZXZ"]
)
code0 = codeTN(["z"])
codePlus = codeTN(["x"])
code922 = codeTN(
