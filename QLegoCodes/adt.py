    ...
    def trace(self, other_tensor, label1, label2):
        ...
        if this.matrix[1, self.n] == 0:
            ...
            if other.matrix[1, other.n] != 0:
                ...
                M[0, this.n - 1 : width] = (
                    this.matrix[0, 0] * other.matrix[0, 1 : other.n]
                    + this.matrix[0, this.n] * other.matrix[1, 1 : other.n]
                )
                M[0, width : width + this.n - 1] = this.matrix[0, this.n + 1 :]
                M[0, width + this.n - 1 :] = (
                    this.matrix[0, 0] * other.matrix[0, other.n + 1 :]
                    + this.matrix[0, this.n] * other.matrix[1, other.n + 1 :]
                )
                M[1 : this.matrix.shape[0], 0 : this.n - 1] = this.matrix[
                    1:, 1 : this.n
                ]
                M[1 : this.matrix.shape[0], width : width + this.n - 1] = this.matrix[
                    1:, this.n + 1 :
                ]
                M[this.matrix.shape[0] :, this.n - 1 : width] = other.matrix[
                    2:, 1 : other.n
                ]
                M[this.matrix.shape[0] :, width + this.n - 1 :] = other.matrix[
                    2:, other.n + 1 :
                ]
                print(M)
            else:
                ...
                f = (
                    1
                    if this.matrix[0, 0] == other.matrix[0, 0]
                    and this.matrix[0, this.n] == other.matrix[0, other.n]
                    else 0
                )
                ...
                M[1 : this.matrix.shape[0], 0 : this.n - 1] = this.matrix[
                    1 : this.matrix.shape[0], 1 : this.n
                ]
                M[1 : this.matrix.shape[0], width : width + this.n - 1] = this.matrix[
                    1 : this.matrix.shape[0], this.n + 1 :
                ]
                M[this.matrix.shape[0] :, this.n - 1 : width] = other.matrix[
                    1 : other.matrix.shape[0], 1 : other.n
                ]
                M[this.matrix.shape[0] :, width + this.n - 1 :] = other.matrix[
                    1 : other.matrix.shape[0], other.n + 1 :
                ]
        else:
            if other.matrix.shape[0] > 1 and other.matrix[1, other.n] != 0:
                ...
                M[2 : this.matrix.shape[0], 0 : this.n - 1] = this.matrix[
                    2:, 1 : this.n
                ]
                M[2 : this.matrix.shape[0], width : width + this.n - 1] = this.matrix[
                    2:, this.n + 1 :
                ]
                M[this.matrix.shape[0] :, this.n - 1 : width] = other.matrix[
                    2:, 1 : other.n
                ...
                M = np.zeros((height, 2 * width), dtype=np.int16)
                M[0, 0 : this.n - 1] = (
                    i * this.matrix[0, 1 : this.n] - j * this.matrix[1, 1 : this.n]
                )
                M[0, this.n - 1 : width] = other.matrix[0, 1 : other.n]
                M[0, width : width + this.n - 1] = (
                    i * this.matrix[0, this.n + 1 :] - j * this.matrix[1, this.n + 1 :]
                )
                M[0, width + this.n - 1 :] = other.matrix[0, other.n + 1 :]
                M[1 : this.matrix.shape[0] - 1, 0 : this.n - 1] = this.matrix[
                    2 : this.matrix.shape[0], 1 : this.n
                ]
                M[1 : this.matrix.shape[0] - 1, width : width + this.n - 1] = (
                    this.matrix[2 : this.matrix.shape[0], this.n + 1 :]
                )
                M[this.matrix.shape[0] - 1 :, this.n - 1 : width] = other.matrix[
                    1 : other.matrix.shape[0], 1 : other.n
                ]
                M[this.matrix.shape[0] - 1 :, width + this.n - 1 :] = other.matrix[
                    1 : other.matrix.shape[0], other.n + 1 :
                ]
        this.matrix = M
        this.Mod2()
